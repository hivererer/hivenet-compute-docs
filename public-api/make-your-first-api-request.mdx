...
This page uses `GET /instances` because it’s a safe way to confirm that your API token works, your base URL is correct, and your client can read a JSON response.
...
| `/instances` | Lists Compute instances visible to your account. |
...
| `instance_id` | The unique ID of an instance. You’ll use this for instance-specific requests. |
...
<Warning>
  Don’t test the API for the first time with a destructive action. Use read-only endpoints such as `GET /instances` until you’re sure your token and client are working. 
</Warning>

## Next step

After your first request works, continue with **Authenticate API requests** to learn how bearer tokens are sent, stored, and handled safely.