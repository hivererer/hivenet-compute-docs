...
<Note>
  An error response does not always mean the API is unavailable. Many errors mean the request needs a different token, parameter, resource ID, or retry strategy.
</Note>
...
For example, this request may fail if `status` does not match one of the supported filter values:
...
For example, `GET /instances/{id}` may return `404` if the instance ID is wrong, or if the instance is not visible to the user or organization linked to the token.
...
- Try listing resources first, then copy the ID from the list response.

## Handle conflict errors

A `409 Conflict` means the request cannot be completed because the resource is not in the right state for that action.
...
| :-- | :-- |
| Read-only requests, such as `GET /instances` | Usually safe to retry after a short delay. |
...
<Steps>
  ...
  <Step title="Check resource state after actions">
    For actions that change resources, fetch the resource again before deciding whether another request is needed.
  </Step>
</Steps>
...
- The endpoint path, such as `GET /instances`
