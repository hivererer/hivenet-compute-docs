...
description: "Page through Compute API list results with cursor pagination and narrow responses using supported filter query parameters like status and region."
...
| Filter | What it does |
| :-- | :-- |
| `status` | Filters by a lifecycle-status bucket. `running` also includes `CREATED`, `DEPLOYED`, and `STARTING`; `stopped` includes `STOPPING`; `terminated` includes `TERMINATING`; `errored` matches `ERRORED`. |
| `gpu_type` | Filters by GPU model. |
| `region` | Filters by region. |
| `date_range_begin` | Returns instances that started at or after a timestamp. |
| `date_range_end` | Returns instances that started at or before a timestamp. |
...
Date filters apply to `started_at`, not the time the instance record was created. An instance that has never started is excluded as soon as either date bound is used.
