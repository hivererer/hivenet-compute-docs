...
Use a read-only endpoint to test authentication.
...
- `GET /instances` may return only instances visible to your account.
- `GET /instances/{id}` may return `404` if the instance doesn’t exist or isn’t visible to your token.
- Actions such as starting or terminating an instance may require access to that instance.\
...
| `404 Not Found` | The resource wasn’t found or isn’t visible to the token. | Check the resource ID and confirm the token can access it. |
