...
description: "Launch your first GPU or CPU instance, or a vLLM inference server, on Compute with Hivenet in minutes: sign up, add an SSH key, and connect."
...
<Steps>
  ...
  <Step title="Create a GPU or CPU instance">
    ...
    1. Choose what you want to create: **Container **or **Virtual machine**.
    2. **Choose a location.** Pick what’s closest to your users or data.
    3. **Choose your setup.** Select **GPU** or **vCPU only**.
    4. **Pick a template or OS.** Examples include **Ubuntu 24.04 LTS**, **PyTorch 2.5**, or **Vulkan**. Templates and OSes come pre-installed, so you can start faster.
    5. **Connectivity.**
    ...
    6. Review the summary and select ****Create instance ****or ****Create virtual machine.****
...
- **Can’t reach the service**\
  Check that the required **port** is enabled under **Connectivity** and that any firewall rules are correct.
