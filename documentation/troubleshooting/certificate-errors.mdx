...
Users may sometimes see certificate warnings when opening `https://console.hivecompute.ai` or calling the Compute API. This usually means something on the local network is intercepting HTTPS traffic. The certificates on Hivenet’s side are valid. The issue almost always comes from corporate firewalls, antivirus software, or SSL inspection tools that replace certificates during traffic scanning.
