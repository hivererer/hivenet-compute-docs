...
description: "Learn how Compute organizations handle people, roles, shared credits, instances, and billing."
icon: "building"
---
...
<Info>
  An organization can contain one **Owner**, one or more **Admins**, and one or more **Members**. The Owner controls billing and ownership. Admins help manage the organization. Members use its resources.
</Info>
...
The organization page shows its Owner, Admins, Members, and pending invitations. The actions available on the page depend on your role.
...
<Note>
  When a person accepts an invitation to another organization, any remaining credits associated with their current individual account are transferred to the organization they join. Future resources are billed to the new organization balance.
</Note>
...
- They may need to terminate running or stopped instances before they can join.
...
Admins and Members can leave an organization from the organization page. Before leaving, terminate any running or stopped instances you created if Compute asks you to do so.
...
The Owner cannot leave directly. They must transfer ownership to another Admin or Member first.
